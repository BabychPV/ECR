...
public sealed class HttpLoadBenchmark
{
        ...
        List<string> notes)
    {
        ...
        {
            notes.Add(Fmt($"""
                Планів «MERGE doc.CellValue» у кеші: {plans:F0}. Очікувано > 1 і саме це WR-01:
                    параметри Decimal/DateTime2 оголошені без Precision/Scale (NormalizedCellStore.cs:588-593),
                    тому сигнатура запиту змінюється від САМИХ ЧИСЕЛ, а текст — ще й від розміру чанка.
                    Після WR-01 має лишитися по одному плану на розмір чанка, після WR-02 (TVP) — один.
                """));
        }
