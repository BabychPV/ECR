...
> **Два різні `Sid`, які не можна плутати (D-86).**
> `sec.User.Sid` — **ідентифікатор** доменного користувача (заповнений лише при
> `Provider = Windows`), а `PrincipalSid` — ідентифікатор AD-групи для
> призначення ролі. Обидва лишаються.
> **Автором дії** ж скрізь є `…ByUserId` → `sec.User(Id)`, а не SID: у локальних
> облікових записів SID немає, а модель вимагає рівноправності провайдерів.
...
| `LastLoginAt`, `CreatedAt/ByUserId` | | |
...
`ClientIp`, `UserAgent`, `RevokedAt`, `RevokedByUserId`
...
| `AssignedAt/ByUserId`, `RevokedAt/ByUserId` | | |
...
`UserCreated`/`ImpersonationStarted`), `ActorUserId`, `TargetUserId` NULL,
