...
| Q-159 | CONFLICT | періоди щойно створеного проєкту лишаються `Scheduled` до наступної години: `PeriodStateJob` виконується на старті застосунку і щогодини, тож новий проєкт не приймає жодного запису | OPEN |
...
| Q-161 | CONFLICT | два сценарії (`IsDeny_грант_блокує_новий_рядок`, `Заархівований_проєкт_блокує_новий_рядок`) досі адресують `documentId=1`/`tableInstanceId=1` — числа, чия відсутність у спільній базі не гарантована, а лише випадкова на сьогодні | OPEN |
...
| Q-163 | CONFLICT | `cfg.FormulaDef` має ДВА зовнішні ключі на `cfg.TableDef` — оголошений `TableDefId` і тіньовий `TableDefId1`; публікація і кеш метаданих читають РІЗНІ, тож формула, вставлена лише з `TableDefId`, для публікації не існує | OPEN |
