...
На Етапі 2 не заблокувало: структуру шаблону віддає
`GetTemplateStructureHandler`, який на цьому етапі не чіпали
...
**Що потрібно від людини:** підтвердити −4 (тоді EBNF §1 і таблиця §2
виправляються) або обрати 4 (тоді змінюється `InlineData` у `06b`, а разом із
ним — очікування, з якими звірятимуть числа).
