...
| Q-217 | CONFLICT | Реальний прогін людиною `rebuild-and-package-msi.ps1`: `npm ci` впав з `NativeCommandError` на самому виклику (до власної перевірки `$LASTEXITCODE`) через звичайне попередження `npm warn deprecated` у stderr — не помилку, код виходу був 0. Причина системна: PowerShell 7.3+ `$PSNativeCommandUseErrorActionPreference` за замовчуванням `$true` підпорядковує ЛЮБИЙ stderr-запис нативної команди `$ErrorActionPreference`, і всі 10 скриптів `tools/*.ps1` з `$ErrorActionPreference = 'Stop'` мали цю вразливість, не лише той, що впав | RESOLVED · `$PSNativeCommandUseErrorActionPreference = $false` додано в усі 10 скриптів одразу після `$ErrorActionPreference = 'Stop'` — єдине джерело істини про успіх нативного виклику лишається явний `$LASTEXITCODE`, як і було задумано скрізь. Перевірено парсером (`ParseFile`, 10/10 без помилок); не перевірено реальним прогоном — `pwsh` 7+ не встановлений у цьому середовищі (лише Windows PowerShell 5.1, де самої вразливості й немає), тож потрібне підтвердження людини на власній машині |
...
### Q-217 · CONFLICT · Реальний прогін людиною, 2026-09-10 · `NativeCommandError` на звичайному попередженні `npm warn` — вразливі всі 10 скриптів `tools/`, не один
...
`.\tools\rebuild-and-package-msi.ps1 -Version 1.0.0` на власній машині.
Публікація `Ecr.Api` й перебудова залежних проєктів пройшли чисто (0
Error(s)), MSI-пакування дійшло до збірки веб-клієнта — і впало:
...
`npm ci` не впав — це звичайне попередження про застарілу транзитивну
залежність, код виходу `0`. Але виняток кинуло на **самому виклику**
(рядок 90), ДО того, як власна перевірка скрипта
`if ($LASTEXITCODE) { throw ... }` (рядок 91) взагалі встигла
відпрацювати.

**Причина — системна, не в одному скрипті.** PowerShell 7.3+ увів
`$PSNativeCommandUseErrorActionPreference`, за замовчуванням `$true`:
будь-який запис нативної команди в stderr підпорядковується
`$ErrorActionPreference`, незалежно від фактичного коду виходу. Усі 10
скриптів `tools/*.ps1`, що ставлять `$ErrorActionPreference = 'Stop'`,
мали цю вразливість — не лише `build-msi.ps1`, якому не пощастило впасти
першим. Кожен, хто викликає `sqlcmd`/`msiexec`/`dotnet`/`npm`/`signtool`
і отримує звідти хоч рядок у stderr (попередження, не обов'язково
помилку), ризикував тим самим передчасним падінням.

**Що зроблено.** `$PSNativeCommandUseErrorActionPreference = $false`
додано в усі 10 скриптів одразу після `$ErrorActionPreference = 'Stop'`.
Це не послаблює перевірку помилок — навпаки, повертає її до того, що
скрипти й так уже робили явно: єдине джерело істини про успіх нативного
виклику — `$LASTEXITCODE`, перевірений `if`-ом одразу після виклику
(`build-msi.ps1:91`, `deploy-ecr.ps1` — та сама форма скрізь). Вимкнення
неявної поведінки нічого не приховує — воно прибирає ДРУГЕ, конфліктне
джерело істини, яке саме й дало хибне падіння.

**Доказ.** `[System.Management.Automation.Language.Parser]::ParseFile`
на всіх 10 файлах — 0 синтаксичних помилок.

⚠ **Не перевірено реальним прогоном.** `pwsh` (PowerShell 7+) не
встановлений у середовищі, де це виправлялося — лише Windows PowerShell
5.1, де самої вразливості немає взагалі (`$PSNativeCommandUseErrorAction
Preference` — поняття, якого 5.1 не знає, тож рядок фікса там просто
безрезультатний, не шкідливий). Синтаксис підтверджено; те, що фікс
справді усуває симптом зі скріншота — підтвердить лише повторний прогін
на машині, де це сталося вперше.

**Статус:** RESOLVED · фікс застосовано в усіх 10 скриптах і перевірено
парсером; реальний прогін `rebuild-and-package-msi.ps1` на машині, де
це сталося вперше, — підтвердження від людини, не від цієї сесії.
