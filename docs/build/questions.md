...
| Q-230 | CONFLICT | Той самий реальний прогін (Q-228, наступний день): SQL Login (`sa`) і потім Windows Authentication ОБИДВА впали на кроці 3 з голим "Не вдалося підключитися... Перевірте назву інстансу, автентифікацію й мережевий доступ" — БЕЗ жодного діагностичного рядка, хоча `Data Link Properties` (Windows-компонент, `Test Connection`) до ТОГО САМОГО сервера тим самим Windows-логіном — успішний. `SqlPreflight.TryVerifyDatabaseExists` читав для показу лише STDERR дочірнього sqlcmd; код виходу (`process.ExitCode`) використовувався лише для гілкування, ніколи не показувався | Частково RESOLVED · причина ПОРОЖНЬОГО повідомлення виправлена: тепер завжди показується код виходу sqlcmd, і STDOUT читається й показується поруч зі STDERR (раніше — лише STDERR, а причина відмови на цій машині могла піти в STDOUT або взагалі нікуди — обидва варіанти тепер видно, а не мовчать). Додано `-I` до аргументів sqlcmd для точної відповідності виклику `deploy-ecr.ps1`. Свідомо НЕ виправлено: чому саме sqlcmd (ODBC) не з'єднується там, де Windows OLEDB (`Data Link Properties`) з'єднується — цього я не знаю (можливі причини: TCP/IP вимкнено в SQL Server Configuration Manager при увімкненому Shared Memory, якого сервер OLEDB використовує для локального з'єднання, а sqlcmd — ні; розбіжність версій ODBC-драйвера; SQL Server Browser не запущений для named instance) — потребує повного діагностичного тексту з НАСТУПНОГО реального прогону з цим фіксом, а не здогаду |
