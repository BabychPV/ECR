...
| Q-217 | CONFLICT | Реальний прогін людиною `rebuild-and-package-msi.ps1`: `npm ci` впав з `NativeCommandError` на самому виклику (до власної перевірки `$LASTEXITCODE`) через звичайне попередження `npm warn deprecated` у stderr — не помилку, код виходу був 0. **Перший фікс сесії (нижче в тілі запису) виявився no-op** — людина повторила прогін, і та сама помилка стався знову з ідентичним фіксом на диску | RESOLVED · виправлено вдруге, `$ErrorActionPreference = 'Continue'` тимчасово на час кожного нативного виклику (усі 10 скриптів) — не через `$PSNativeCommandUseErrorActionPreference`. Перевірено парсером (10/10) і логікою функції на Windows PowerShell 5.1 (`Invoke-NativeStep` пропускає нешкідливий stderr, ловить справжню помилку); реальний прогін саме цього сценарію на pwsh 7+ — за людиною, `pwsh` не встановлений у середовищі, де це виправлялося |
