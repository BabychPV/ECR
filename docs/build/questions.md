...
## Q-153. Періоди щойно створеного проєкту лишаються `Scheduled` до наступної години
...
рівно одна задача, `PeriodStateJob`; `RecurringScheduleService` запускає її один
раз на старті застосунку і далі щогодини (`HourlyCron`). Проєкт, створений
ПІСЛЯ старту, до наступної години не має жодного періоду, у який можна писати.
...
того, як календар побудовано: його старт і вирівнює стани.

#### Чому не виправив

Поза обсягом `W6`. Це не дефект методологій, а поведінка планувальника станів
періодів: правильне виправлення — вирівнювати стани при АКТИВАЦІЇ проєкту (там,
де людина натиснула кнопку і чекає), а не покладатися на годинний cron. Це
торкається `ProjectsController.Activate`, `PeriodStateJob` і, ймовірно, самого
...
тоді активація стає довшою, зате період відкривається одразу), чи лишити як є і
вважати, що новий проєкт годину не приймає даних.

---

## Q-154. `S-21` недосяжний: формули шаблону і методологій — два різні конвеєри
...
формул шаблону. Робити його зеленим, не довівши того конвеєра, означало б
