...
Базу створюють DBA замовника **до** запуску будь-якого скрипта з цього пакета.
Зіставлення при цьому задається **явно**:

```sql
CREATE DATABASE [ECR] COLLATE Latin1_General_100_CI_AS_SC;
```

⚠ Це не питання сортування. Зіставлення визначає, чи `UQ_Template_Code`,
`UQ_Unit_Code`, `UQ_Role` і `UQ_RegistryDef` вважають `ABC` і `abc` **одним
кодом**. При `CI` — так, при `CS` — ні, і той самий seed на двох інстансах дає
різний результат: на одному запис вставиться, на другому впаде на порушенні
унікальності. Отримати це «за замовчуванням від інстансу» означає зробити
поведінку унікальності бізнес-кодів властивістю чужої машини (`Q-061`).

Чому саме `Latin1_General_100_CI_AS_SC`:

| Частина | Що дає |
|---|---|
| `Latin1_General_100` | нейтральне до мови даних; увесь текст усе одно `nvarchar` |
| `CI` | **обов'язкова** — її очікує решта системи |
| `AS` | наголоси розрізняються: `Ü` і `U` — різні коди |
| `SC` | коректна робота з сурогатними парами в `nvarchar` |

Мова інтерфейсу тут ні до чого: локалізовані рядки живуть у
`sys_ecr.UiString` і `cfg.LocalizedText` і сортуються застосунком, а не базою.

`SchemaValidator` при старті **попереджає**, якщо зіставлення виявилося `CS`.
Це попередження, а не зупинка, з тієї ж причини, що й RCSI: змінити зіставлення
бази застосунок не має права.
