...
## Смуги фази 3, 2026-09-09 (паралельно, без перетину файлів)

Настанова людини: десять пунктів знято з ескалації (рішення нижче),
чотири зведено в одне питання людині, Фаза 4 звужена до трьох задач.
Дві смуги йдуть одночасно — жодного спільного файлу.

| Смуга | Порядок | Файли |
|---|---|---|
| **A** (застосунок) | `Q-173` → `Q-156`+`Q-149а` → `Q-168A`+`Q-168B`+`Q-178` → `Q-180` → `Q-155` → `Q-184`/`Q-185` | `src/Ecr.Application/**`, `src/Ecr.Infrastructure/**`, `tests/Ecr.*.Tests/**` |
| **B** (клієнт) | `W4.1` → `W4.2` → `W4.3` | `src/Ecr.Web/**` |

Закрили пункт смуги — приберіть його з переліку тут у тому ж PR.
