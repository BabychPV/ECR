...
| F6 | `Water_070.Rollup.TotalTons` | `CONVERT(([Jan] + [Feb] + [Mar]) * CST_WATER_DENSITY, 'kg', 't')` | **конверсію** і контекстний коефіцієнт |
...
`CST_WATER_DENSITY` = `1000` `kg_per_m3` — константа методології, **не**
конверсія (`ФВ-16.5`).
...
```json
{
        ...
        "tables": [
          {
            ...
            "formulas": [
              ...
              { "id": "F6", "scope": "Cell", "row": "C009", "column": "TotalTons",
                "expression": "CONVERT(([Jan] + [Feb] + [Mar]) * CST.CST_WATER_DENSITY, 'kg', 't')" }
            ]
          }
        ]
      },
  ...
  "expected": {
    ...
    "calculation": {
      "7001001": {
        "SUB-COD": { "tons": "0.912688", "gsec_Actual": "0.340774", "gsec_Fixed360": "0.352117" },
