...
### `src/Ecr.Web/src/shared/__tests__/formula-budget.test.ts`
MODULE: tests-web | STAGE: 6
...
/**
 * Чому клієнтський обчислювач НЕ має бюджету кроків, який має серверний
 * (`Evaluator.MaxEvaluationSteps = 20 000`, `02b` §6.5–6.6).
 *
 * ⛔ Тут доводиться НЕДОСЯЖНІСТЬ, а не наявність захисту. Вираз, заради якого
 * заведено серверний бюджет (тридцять предикатних агрегатів, 1587 символів,
 * 16 820 мс на сервері), коштує клієнтові 0.3 мс і дає `#NAME`: клієнт не
 * резолвить посилань на комірки, тобто до даних — єдиного, що робить вираз
 * дорогим, — не доходить узагалі. Лишається ціна за довжиною тексту, і вона
 * лінійна (замір: 400 000 символів — 152 мс).
 *
 * ⚠ Бюджет «про всяк випадок» був би ГІРШИМ за його відсутність: він мусив би
 * спрацьовувати рівно там, де серверний, інакше підказка показала б число
 * там, де в документ поїде `#BUDGET`.
 */
...
⚠ Друга половина цього доказу живе на СЕРВЕРІ —
`ClientServerEquivalenceTests.Найдорожчий_клієнтський_вираз_не_наближається_до_бюджету_сервера`
і `…Жоден_вираз_спільного_набору_не_наближається_до_бюджету_сервера`. Обидва боки
будують той самий вираз із поля `budgetEdge` спільної фікстури і звіряють його
довжину: клієнт міряє час, сервер — витрачені кроки (999 із 20 000).
...
| Бюджет обчислення | що межа сервера недосяжна з боку клієнта — тобто підказка не покаже числа там, де сервер віддасть `#BUDGET` |
