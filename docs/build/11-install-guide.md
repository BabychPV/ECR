...
- SQL Server ~~(Express достатньо)~~ — **потрібна редакція з SQL Server
  Agent** (Standard і вище; Developer — для стенда). Базу майстер/`deploy-ecr.ps1`
  **створює сам**, якщо її ще немає (`-CreateDatabaseIfMissing`, директива людини —
  `docs/build/10-installer.md` §1.3) — потрібне лише право `CREATE
  ...
  > запуститься і рік-два працюватиме, а потім почне мовчки псувати дані.
  > Ланцюг перевірено в коді:
  >
  > 1. `src/Ecr.Infrastructure/Persistence/Sql/02-partitions.sql:24-32` —
  >    межі `pf_ByPeriodKey` виписані наперед рівно до `202712`
  >    (`RANGE RIGHT FOR VALUES (202601 … 202712)`);
  > 2. рухає межі вперед лише `arc.usp_EnsurePartitions`
  >    (`04-partition-maintenance.sql:15`), і єдиний її викликач у всьому
  >    репозиторії — завдання SQL Server Agent `ECR: Partitions ahead`
  >    (`14-agent-jobs.sql:61,69`; перевірено `git grep usp_EnsurePartitions`
  >    по `src/`, `tools/`, `installer/` — третього викликача немає);
  > 3. SQL Server Express **не має Agent взагалі**, тож із січня 2028 усі
  >    періоди лягають в останню партицію — без помилки, без запису в журнал.
  >
  > ⛔ Сторож, який мав би це зупинити, не спрацює:
  > `14-agent-jobs.sql:29-30` перевіряє `IF DB_ID('msdb') IS NULL`, а `msdb`
  > є в **усіх** редакціях, включно з Express. Скрипт пройде, завдання
...
> ✎ **2026-09-18:** переживає саме **тека**, а не логи — писати в неї нема
> кому, файлового логера в застосунку немає (§7, `R-02`). Формулювання
> лишається перекресленим, бо рівно з нього й починається хибний висновок
> «логи десь є».
...
| ~~Лог застосунку (Serilog/файловий)~~ — **не існує, див. нижче** | ~~`%ProgramData%\ECR\logs\`~~ |
| Системний Event Log | ~~Джерело `ECR`~~ → джерело **`Ecr.Api`**, канал `Application`, рівень `Warning` і вище |

> ⛔ **✎ 2026-09-18 (директива №14 §9, `R-02`/`R-03`). Обидва рядки вище
> брехали, і саме тому були найдорожчими в таблиці: адміністратор, у якого
> щось не так, іде за ними першими — у порожню теку.** Перевірено в коді:
>
> - **файлового логера немає взагалі.** `src/Ecr.Api/Ecr.Api.csproj:20-25` —
>   чотири `PackageReference`, жодного `Serilog.*` серед них;
>   `src/Ecr.Api/Program.cs` не містить ані `builder.Logging`, ані
>   `AddFile`, ані будь-якої згадки Serilog (`git grep -iE "Serilog|AddFile" -- src/Ecr.Api`
>   — нуль збігів). Теку `%ProgramData%\ECR\logs` інсталятор створює
>   (`10-installer.md:286`), і в неї ніхто ніколи нічого не пише;
> - **джерело Event Log не те, яке зареєстровано.** MSI реєструє джерело
>   `ECR` (`installer/Ecr.Installer/Service.wxs:74`:
>   `<util:EventSource Log="Application" Name="ECR" …>`), а застосунок пише
>   під назвою застосунку — `Program.cs:22` викликає
>   `builder.Host.UseWindowsService()`, і провайдер Event Log, який той
>   додає, бере `SourceName` за замовчуванням, тобто `Ecr.Api`. Ніде в `src/`
>   немає ані `AddEventLog`, ані `EventLogSettings`, ані `SourceName` —
>   отже перевизначення джерела теж немає.
>
> ⚠ Практичний наслідок, поки `D14-09` не зроблено: під Windows-службою
> консолі не існує, файлу немає, тож **стек винятку з `CorrelationId` не
> зберігається ніде**. У журналі `Application` осідає лише те, що пройшло
> дефолтний фільтр провайдера (`Warning` і вище), і шукати це треба за
> джерелом `Ecr.Api`, а не `ECR`. Повний текст помилки з кореляцією
> відтворюється тільки запуском `Ecr.Api.exe` з консолі.
>
> ⚠ `tools/deploy-ecr.ps1:661` теж посилає адміністратора у `%ProgramData%\ECR\logs`
> — це той самий дефект, і виправляється він **разом** зі скриптом (`D14-09`),
> не цією правкою документа.
