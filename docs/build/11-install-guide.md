...
>
> **Два шляхи, той самий результат:** майстер `EcrSetup.exe` (розділ
> 2.1, рекомендовано — п'ять екранів замість параметрів команди) або
> `deploy-ecr.ps1` напряму (розділ 2.2 — для автоматизації/CI). Розділи
...
**Майстер встановлення** (`EcrSetup.exe`, розділ 2.1) — окремий проєкт,
поза `Ecr.sln` (Windows-only GUI, той самий прецедент, що `.wixproj`
поряд, `Q-216`). Збирається окремо:
...
⚠ `EcrSetup.exe` шукає `deploy-ecr.ps1` **поруч із собою** (`InstallStep.
ResolveScriptPath`), тому копіювати скрипт в ту саму теку — не
формальність. Публікація вище — framework-dependent (без
`-SelfContained`): на сервері має стояти .NET 10 Desktop Runtime,
або опублікуй з `--self-contained -r win-x64`, якщо це не гарантовано.

⛔ **Скопіювати на сервер:** `Ecr.msi` і, якщо плануєш ставити через
майстер (рекомендовано, розділ 2.1), увесь вміст `artifacts\setup\`
(`EcrSetup.exe` + `deploy-ecr.ps1` поруч) — в ОДНУ теку: майстер сам
знаходить `.msi`, що лежить поруч із ним. Далі всі дії — вже на сервері.
...
Запустити `EcrSetup.exe` **від імені адміністратора** (маніфест вимагає
це сам — Windows перепитає, якщо забув). П'ять екранів по черзі, одне
питання за раз:
