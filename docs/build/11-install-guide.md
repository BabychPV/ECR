...
| Лог застосунку (файловий, Serilog) — **✎ 2026-09-20: існує, див. «Файловий журнал» нижче** | `%ProgramData%\ECR\logs\ecr-yyyyMMdd.log`; фактичну теку показує `/admin/health` → «Log directory» |
...
>
> ✎ **2026-09-20 (рішення №10 директиви №15). Перший пункт і «практичний
> наслідок» вище ЗАСТАРІЛИ: файловий журнал є.** Запис не видалено — він
> пояснює, чому на встановленнях до цієї версії тека порожня. Про джерело
> Event Log (`Ecr.Api`, не `ECR`) усе сказане лишається чинним: цю половину
> `D14-09` ще не зроблено. Посилання `deploy-ecr.ps1` на теку логів відтепер
> правдиве.
...
- **Де:** `%ProgramData%\ECR\logs\ecr-yyyyMMdd.log` (теку створює MSI; обліковий
  запис служби має на неї повні права — `Folders.wxs`). Фактичну теку, в яку
  процес справді пише, показує `/admin/health` → «Log directory»; рядка немає —
  файл не пишеться.
- **Ротація:** новий файл щодоби; файл понад 100 МБ перекочується в
  `ecr-yyyyMMdd_001.log`. Зберігаються **30 останніх файлів**, старіші
  видаляються самі.
- **Що в рядку:** час, рівень, `[CorrelationId]`, `[uid:<числовий UserId>]`,
  `[ім'я машини]`, категорія, повідомлення, стек винятку. `CorrelationId` — той
  самий, що у відповіді з помилкою (`X-Correlation-Id`), тож шукати:
  `Select-String -Path "$env:ProgramData\ECR\logs\ecr-*.log" -Pattern "<correlationId>"`.
  Логінів, SID, паролів, тіл запитів і рядка підключення в журналі немає.
- **Рівні** — секція `Logging:LogLevel` (одна на консоль, Event Log і файл).
- **Як змінити** — у `%ProgramData%\ECR\config\appsettings.Production.json` або
  змінними оточення служби (`ECR_Logging__File__Directory`):

  ```json
  { "Logging": { "File": { "Directory": "D:\\Logs\\ECR", "RetainedFiles": 30, "FileSizeLimitMb": 100 } } }
  ```

  Змінні оточення в шляху розгортаються; відносний шлях рахується від теки
  `Ecr.Api.exe`; порожній рядок вимикає файл. Обліковому запису служби потрібне
  право запису в нову теку.
- **Якщо в теку не вдається писати** — служба стартує й працює БЕЗ файлу, а в
  Event Log (джерело `Ecr.Api`) лягає попередження «Файловий журнал вимкнено…» із
  причиною.
