...
⚠ Смуга **B** (клієнт, `W4.1` → `W4.2` → `W4.3`) закрита цілком —
`W4.1` (`PR #86`), `W4.2` у трьох PR (`#88`, `#90`, `#91`), `W4.3`
(`PR #94`). Рядок прибрано за правилом нижче («закрили пункт смуги —
приберіть його з переліку тут у тому ж PR»). Один залишок поза
`src/Ecr.Web/**`: підключення матриці `light`/`dark` у джобі `a11y`
`.github/workflows/ci.yml` — файл поза межами смуги B, винесено окремим
завданням (`task_b634f02b`).
