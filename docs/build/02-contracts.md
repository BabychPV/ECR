...
⚠ Мітку постановки несе `JobDataMap` самої задачі, а не база: `itg.JobProgress`
з'являється вже зі станом `Running`, тобто в момент старту, а стан `Queued`
фігурує у фільтрах `JobProgressStore` і ніким не пишеться. Записувати рядок при
постановці означало б додати похід до СУБД на шлях, який сам і міряється.
