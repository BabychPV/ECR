...
⚠ **`BE-03`.** Звуження журналу передається ЗАПИСОМ `CellChangeFilter`
(`From`, `To`, `DocumentId?`, `RowKey?`, `ColumnDefId?`, `ChangedByUserId?`,
`Origin?`, `LateOnly`), а не списком позиційних параметрів: чотири підряд
необов'язкові ідентифікатори, два з яких `int`, переставляються місцями мовчки
і дають журнал ЧУЖОЇ комірки.

⛔ Вікно `From`…`To` лишається **обов'язковим** — це поля запису, а не
необов'язкові параметри. `aud.CellChange` партиційована за `ChangedAt`, і
запит без меж іде по ВСІХ партиціях. Замір на 600 000 рядків у 24 місячних
партиціях (2026-09-19): вікно 92 дні — 4 партиції й 1 304 логічних читання;
той самий запит без вікна — 25 партицій і 10 346 читань. Фільтри `author`,
`origin`, `lateOnly` **не додають жодного читання** понад вікно (ті самі
4 партиції / 1 304 читання): вони звужують уже відсічений вікном набір.

⚠ **Два рівні доступу на `GET /api/v1/audit/cells`** (`D15-16`). Таблиця §9
називає `Security.ViewAudit` — це поріг ЗАГАЛЬНОГО журналу, і він не
змінився. Але коли фільтр задає ПОВНУ адресу однієї комірки
(`documentId` + `rowKey` + `columnDefId`), достатньо `Document.View` плюс
той самий грант на проєкт, що й для читання зрізу: питання «що було з ЦИМ
числом» — це питання про документ, який людина й так бачить. Роздавати
комплаєнс-право `Security.ViewAudit` заради вкладки History в інспекторі
комірки означало б віддати разом із нею наскрізний журнал по всіх проєктах.
Стеля вікна для однієї комірки — 13 місяців (396 днів) замість 92 днів;
для журналу стеля не послаблена.
...
Затримка «поставили задачу в чергу → задача ПОЧАЛА виконуватись» (`ФВ-12.2`,
`tz/08` §8.3). Той самий прийом, що `IConsistencyMetrics`: місток задач живе в
`Ecr.Infrastructure`, метрики — в `Ecr.Api`.

⛔ Це **не** `ecr.job.duration`. Та міряє, скільки задача працювала; ця —
скільки вона чекала, поки її візьмуть. Величини незалежні: перерахунок може
йти 200 мс, а починатися через хвилину.

⚠ Мітку постановки несе `JobDataMap` самої задачі, а не база. Момент
постановки в базі **є** — `QueueAsync` створює рядок `itg.JobProgress` зі
станом `Queued` ще до старту, — але він **не переживає старту**:
`JobProgress.Begin` перезаписує `UpdatedAt`, а `StartedAt` ставить на мить
запуску. Читати з бази довелося б рівно в мить переходу; мітка в `JobDataMap`
не додає походу до СУБД на шлях, який сам і міряється, і не залежить від
...
| `ECR-CALC-0438` | 422 | аргумент, який формула версії вживає, не відповідає жодній колонці таблиці, до якої `CalculationBinding` прив'язує методологію (директива «структурна перевірка аргументів методології при публікації») |
...
| `ecr.job.start_latency` | Histogram (ms) | **бюджет `< 10 с` авто / `< 5 с` ручний** (`ФВ-12.2`, `tz/08` §8.3); тег `job` |
