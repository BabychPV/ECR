...
Одна команда робить усе: створює тимчасову базу, розгортає в тому самому
порядку, що вище, наповнює `doc.CellValue` до **108 млн** рядків і знімає
шість замірів `B02` §7. **Код виходу 2 означає, що бюджет не витриманий** —
це перевірка, а не звіт.

| Параметр | Типово | Навіщо міняти |
|---|---:|---|
| `-Cells` | `108000000` | обсяг у **рядках `doc.CellValue`**, не в документах: скільки документів дасть 108 млн, залежить від заповненості |
| `-Fill` | `90` | три сценарії гейта — 35 / 60 / 90 % |
| `-LoadSeconds` | `900` | замір №6; менше значення прогін позначає як **неповний гейт** |
| `-Keep` | — | лишити базу, щоб перезняти числа без наповнення |
| `-SkipGenerate` | — | заміряти вже наповнену базу |

⚠ **Місце і час.** 108 млн комірок — це ≈ **5.9 ГБ** на диску (виміряно:
41.7 Б на комірку разом із `doc.TableRow` та індексами) і **години**
наповнення. Скрипт перевіряє вільне місце **до** початку і зупиняється з
розрахунком, скільки комірок сюди влізе, — бо прогін, який упреться в
«operating system error 112» на дев'яностій хвилині, коштує двох годин і
лишає напівнаповнену базу.

⛔ **Редакція, а не ім'я інстансу.** Скрипт відмовляється працювати на
Express (`EngineEdition = 4`): 1410 МБ буферного пулу означають, що замір №6
міряє диск, а не модель (`Q-063`). На цій машині інстанс називається
`SQLEXPRESS`, а видання — **Enterprise Developer** (`EngineEdition = 3`), і
саме тому перевіряється видання.

⛔ **Своя база і тільки своя.** Скрипт створює базу з розширеною властивістю
`Ecr_Br07_Temp` і видаляє лише ту, що цю властивість має. `-Database EcrDev`
не знищить базу розробника: вона позначки не має, і скрипт скаже про це
вголос. Наявну базу з тим самим іменем він не перезаписує взагалі.

⚠ Замір №6 читає лічильник ескалацій блокувань, а SQL Server дає його **на
весь інстанс**. Поки триває прогін, на інстансі не має працювати нічого
іншого — інакше чужі ескалації зарахуються нам.
