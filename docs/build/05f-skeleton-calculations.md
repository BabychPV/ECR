    ...
    NumericPolicy numeric) : ICalculationModule
{
    ...
    public Task<CalculationOutput> ExecuteAsync(CalculationInput input, CancellationToken ct)
        => throw new NotImplementedException(
            ...
            "4) округлення і арифметику застосовувати через numeric — режим Legacy рахує " +
            "   в double за семантикою NCalc 1.3.8 (ФВ-9.9), а звірка йде в поданні " +
            "   колонки (ФВ-9.16);\n" +
...
/// </summary>
/// <remarks>
/// Режим <see cref="NumericMode.Legacy"/> існує **виключно** заради сумісності
/// з числами чинної системи (ФВ-9.9): та сама арифметика — <c>double</c> із
/// семантикою NCalc 1.3.8 — і те саме банківське округлення. Це не «гірший»
/// режим — це умова того, що звірка з еталоном узагалі можлива.
///
/// ⛔ «Побітово» тут стояло і знято директивою №05 §8: рівність вимагається не
/// в усіх сімнадцяти знаках, а **в поданні колонки** (ФВ-9.16). Різниця
/// моменту округлення між режимами теж була вигадана — див. NumericPolicy.
/// </remarks>
