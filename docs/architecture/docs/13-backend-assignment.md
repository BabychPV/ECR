...
|----------|----------|--------|
| 1 | [01-as-is-overview.md](01-as-is-overview.md) | що замінюємо і чому |
...
│  ├─ Ecr.Calculations/           (наступна фаза) ICalculationModule
...
| # | Тема | Що вирішити |
|---|------|-------------|
| 1 | Модель зберігання комірок | нормалізована vs JSON-гібрид — **за результатом прототипу М0** |
| 2 | Стратегія партиціонування | межі партицій, `SWITCH PARTITION` для архівації, план обслуговування |
...
| 10 | Формат аудиту | пакетний запис vs синхронний; retention; партиціонування |
...
| 12 | **Вирівняні індекси при партиціонуванні** | `PeriodId` у PK і всіх unique-індексах `CellValue`/`TableRow`; структура `arc.*` ідентична для `SWITCH PARTITION` (§13.5 п.2) |
...
| # | Питання |
|---|---------|
| 1 | Довідники: master в AF чи в ECR |
| 2 | Чи потрібна зворотна публікація в AF узагалі |
| 3 | Склад даних, що збираються з AF (може скоротитися після перенесення розрахунків) |
| 4 | Як саме лягають розрахункові модулі емісій у схему `calc` |
