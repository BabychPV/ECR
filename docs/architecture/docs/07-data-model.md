...
| `calc` | *(наступна фаза)* Розрахункові модулі емісій |
...
| `IsDeleted`, `DeletedAt`, `DeletedBySid` | | soft delete |
...
`OldValue`, `NewValue`, `ChangedAt`, `ChangedBySid`. Append-only.
...
| `ExternalSettingsJson` | `nvarchar(max)` | налаштування, зібрані з зовнішнього джерела (для ECR — `CALCULATION_START/ENDDATETIME` з AF); **не окремі колонки**, щоб ядро не знало про AF |
| `Status` | `tinyint` | `Draft`/`Active`/`Grace`/`Closed`/`Archived` |
| `ClosedAt`, `ClosedBySid` | | |
...
`Id`, `DocumentId` FK, `SheetDefId` FK, `IsIncluded` `bit`, `IncludedAt`, `IncludedBySid`
...
| **`RowKey`** | `nvarchar(100)` | стабільна ідентичність (з `RowDef.RowKey` або GUID для динамічних) |
| `RowDefId` | `int` FK NULL | для `RowMode = Fixed` |
| `Ordinal` | `int` | порядок відображення |
| `IsDeleted` | `bit` | |
| `RowVersion` | `rowversion` | |

**UQ:** `(TableInstanceId, RowKey)`
...
| `ColumnDefId` | `int` | |
| `PeriodId` | `int` | денормалізовано для partition key |
...
**Кластерний індекс:** `(TableRowId, ColumnDefId)`
**Партиціонування:** по `PeriodId`
**Стиснення:** `PAGE`
...
`TemplateVersionId`, `ApprovedAt/BySid`,
...
`ActorSid`, `ActedAt`, `Comment` `nvarchar(2000)`
...
| `ChangedAt` `datetime2`, `ChangedBySid` `nvarchar(200)` |
...
### aud.DocumentEvent
`Id`, `DocumentId`, `EventType`, `Payload nvarchar(max)`, `OccurredAt`, `ActorSid`
...
`DetectedAt`, `ResolvedAt`, `ResolvedBySid`
...
`Status`, `ErrorMessage`, `TriggeredBySid` NULL (NULL = за розкладом)

### itg.DictionarySyncRun
`Id`, `RegistryDefId` FK, `CollectionRunId` FK, `Added`, `Updated`,
`Deactivated`, `Status`, `ErrorMessage`

### itg.ExternalPublication *(опційно, перехідний період)*
Потрібна лише поки працює зворотна публікація в PI AF (ТЗ §5.3, ІНТ-11; відкрите питання §12 п.10).
...
Дзеркало робочих таблиць: `arc.TableInstance`, `arc.TableRow`, `arc.CellValue`,
`arc.CellChange`. **Структура ідентична** `doc.*` / `aud.*`, відмінності лише фізичні:
...
`RowsMoved`, `ChecksumSource`, `ChecksumTarget`, `Status`, `ErrorMessage`, `TriggeredBySid`

**Правила:**
* перенесення транзакційне, з **звіркою контрольних сум** до і після;
* розбіжність -> `ROLLBACK` + запис у `aud.ConsistencyIssue`, дані не видаляються з джерела;
* читання архіву **прозоре**: репозиторій обирає джерело за `ProjectId`/`PeriodId`;
* `Reopen` архівного року виконує зворотне перенесення тією самою процедурою;
* автоматичного видалення архіву **немає** — тільки вручну, з підтвердженням.
...
| `doc.Document` | 100–500 | звичайна |
| `doc.TableInstance` | 500 × 90 × 12 ≈ **540 000** | звичайна + IX |
| `doc.TableRow` | 540 000 × ~30 ≈ **16 млн** | партиції по `PeriodId` |
| **`doc.CellValue`** | 16 млн × ~7 ≈ **108 млн** | **партиції по `PeriodId`**, стиснення `PAGE` |
| `aud.CellChange` | ~2× від `CellValue` | партиції по місяцях |
| `ext.RawDataPoint` | залежить від частоти збору | партиції по `Timestamp`, retention за політикою |
| **`arc.*`** | накопичувально по роках | columnstore, окрема файлова група |
