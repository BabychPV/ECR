...
public sealed class MethodologyResolver(IMethodologyStore store)
{
    ...
    public Task<IReadOnlyList<string>> MatchRowsAsync(
        ...
        => throw new NotImplementedException(
            ...
            "MatchJson — СТРУКТУРОВАНИЙ предикат, а не вираз: діалект Methodology посилань " +
            "на комірки документів не має (02b §3.4), а третій діалект не створюється (D-92). " +
            "Обчислювати предикат у пам'яті над значеннями рядка (Q-026).");
}
