...
/// <summary>
/// Позиція відновлюваного сканування таблиці, що не вміщається в один прогін
/// (<c>itg.ScanCursor</c>).
/// </summary>
/// <remarks>
/// ⛔ ЧОМУ ЦЕ ВЗАГАЛІ ІСНУЄ. Нічна перевірка осиротілих рядків
/// (<c>OrphanScanner</c>) брала <c>Take(20_000)</c> БЕЗ упорядкування, без
/// курсора й без циклу — один запит на ніч проти таблиці, розрахованої на
/// ~108 млн рядків на рік. Тобто щоночі оглядалося одне й те саме довільне
/// вікно, а решта не перевірялася НІКОЛИ, і задача звітувала про успіх в
/// обох випадках. Перевірка, яка не може провалитися, бо майже не дивиться, —
/// це не перевірка, а рядок у розкладі.
///
/// ⚠ Позиція — це ПОВНИЙ ключ рядка <c>(PeriodKey, Id)</c>, а не самий
/// <c>Id</c>, і не «скільки вже пройшли». Первинний ключ <c>doc.TableRow</c>
/// складений і саме в цьому порядку кластерний, тож курсор цієї форми
/// перетворює кожну пачку на засічку по діапазону (<c>Seek</c>), а не на
/// сканування. Лічильник «скільки пройшли» (<c>OFFSET</c>) не дав би ні
/// засічки, ні стійкості до вставок і видалень — після кожної зміни набору
/// він указував би на інший рядок.
///
/// ⚠ Стан ЖИВЕ В БАЗІ, а не в пам'яті процесу: інстансів застосунку кілька, і
/// ніч, яку відпрацював інстанс A, не має починатися спочатку тільки тому, що
/// наступну ніч узяв інстанс B. З тієї ж причини це ОКРЕМА сутність, а не
/// поле <c>MaintenanceRun</c>: прогін — це подія в журналі, яких багато, а
/// позиція — один довгоживучий стан на сканування.
///
/// ⚠ І це НЕ друге серцебиття. Живість задачі тримає
/// <see cref="JobProgress.HeartbeatAt"/> (<c>Q-331</c>) — курсор про життя
/// процесу не знає нічого й знати не повинен: він рухається лише після того,
/// як пачку вже ЗАПИСАНО, тож обірвана посеред пачки ніч просто повторить цю
/// пачку наступного разу (записи сканера ідемпотентні — ознака рахується з
/// даних, а не накопичується).
/// </remarks>
public sealed class ScanCursor
{
    private ScanCursor() { }

    /// <summary>Заводить курсор на початку набору.</summary>
    /// <param name="scanCode">Код сканування — він же ключ.</param>
    /// <param name="utcNow">Момент створення в UTC.</param>
    public ScanCursor(string scanCode, DateTime utcNow)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(scanCode);

        ScanCode = scanCode;
        PeriodKeyValue = StartPeriodKeyValue;
        RowId = StartRowId;
        UpdatedAt = utcNow;
    }

    /// <summary>
    /// Ключ періоду, ДО якого включно набір уже пройдено.
    /// </summary>
    /// <remarks>
    /// Початок — <c>0</c>: він строго менший за будь-який реальний
    /// <c>PeriodKey</c> (<c>Year*100 + Sequence</c>), тож «нічого не пройдено»
    /// не потребує окремого прапорця й окремої гілки в предикаті.
    /// </remarks>
    public const int StartPeriodKeyValue = 0;

    /// <summary>Рядок, ДО якого включно пройдено. Початок — <c>0</c>.</summary>
    public const long StartRowId = 0;

    /// <summary>Код сканування; ключ.</summary>
    public string ScanCode { get; private set; } = null!;

    /// <summary>Ключ періоду останнього пройденого рядка.</summary>
    public int PeriodKeyValue { get; private set; }

    /// <summary>Ідентифікатор останнього пройденого рядка в цьому періоді.</summary>
    public long RowId { get; private set; }

    /// <summary>Коли позицію востаннє зрушено.</summary>
    public DateTime UpdatedAt { get; private set; }

    /// <summary>Скільки разів набір пройдено від початку до кінця.</summary>
    /// <remarks>
    /// ⚠ Не декорація. Це єдине число, за яким видно, що сканування справді
    /// ОБХОДИТЬ набір, а не тупцює. Курсор, що росте, і курсор, що замикає
    /// цикли, — різні речі: перший міг би вічно повзти набором, який росте
    /// швидше за нього, і жодного повного обходу так і не зробити.
    /// </remarks>
    public int CyclesCompleted { get; private set; }

    /// <summary>Коли завершено останній повний обхід; <c>null</c> — жодного.</summary>
    public DateTime? LastCycleCompletedAt { get; private set; }

    /// <summary>
    /// Зсуває позицію на останній оброблений рядок.
    /// </summary>
    /// <param name="periodKeyValue">Ключ періоду останнього обробленого рядка.</param>
    /// <param name="rowId">Ідентифікатор останнього обробленого рядка.</param>
    /// <param name="utcNow">Момент зсуву в UTC.</param>
    /// <exception cref="ArgumentOutOfRangeException">
    /// Нова позиція не пізніша за поточну.
    /// </exception>
    /// <remarks>
    /// ⛔ Рух СТРОГО вперед, і порушення — виняток, а не тихе ігнорування.
    /// Курсор, який можна відкотити назад, перетворюється рівно на той дефект,
    /// заради якого писався: сканування, що щоночі починає з того самого
    /// місця. Такий відкат — помилка виклику, і вона має бути чутною в тесті,
    /// а не проявитися через рік як «чомусь не покривається хвіст таблиці».
    /// Обернення на початок — окрема дія (<see cref="CompleteCycle"/>), бо це
    /// інша подія: не помилка, а завершений обхід.
    /// </remarks>
    public void AdvanceTo(int periodKeyValue, long rowId, DateTime utcNow)
    {
        if (periodKeyValue < PeriodKeyValue
            || (periodKeyValue == PeriodKeyValue && rowId <= RowId))
        {
            throw new ArgumentOutOfRangeException(
                nameof(rowId),
                rowId,
                $"Курсор '{ScanCode}' може рухатися лише вперед: "
                + $"поточна позиція ({PeriodKeyValue}, {RowId}), задана ({periodKeyValue}, {rowId}).");
        }

        PeriodKeyValue = periodKeyValue;
        RowId = rowId;
        UpdatedAt = utcNow;
    }

    /// <summary>
    /// Фіксує повний обхід набору і повертає курсор на початок.
    /// </summary>
    /// <param name="utcNow">Момент завершення обходу в UTC.</param>
    /// <remarks>
    /// ⚠ Обернення обов'язкове саме тому, що набір ЖИВИЙ. Рядки додаються
    /// позаду курсора (нові періоди дістають менші <c>Id</c> не завжди, а
    /// закриття періоду взагалі прибирає його рядки з набору), і курсор, що
    /// дійшов до кінця й там залишився, більше не оглянув би нічого ніколи.
    /// </remarks>
    public void CompleteCycle(DateTime utcNow)
    {
        PeriodKeyValue = StartPeriodKeyValue;
        RowId = StartRowId;
        CyclesCompleted++;
        LastCycleCompletedAt = utcNow;
        UpdatedAt = utcNow;
    }
}
