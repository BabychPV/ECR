...
public static class ErrorCodes
{
    ...
    /// <summary>
    /// Вичерпано межу спроб входу з однієї адреси (<c>S-10</c>,
    /// <c>LoginRateLimiting</c>).
    /// </summary>
    /// <remarks>
    /// ⛔ Заведений замість запозиченого <see cref="AccountLocked"/>, і це не
    /// косметика. Обмежувач частоти відповідав кодом «обліковку заблоковано»,
    /// а заголовок із каталогу (<c>err.ECR-AUTH-0423</c> — «The account is
    /// locked.») стверджував те саме — при тому що НІЯКОЇ обліковки ніхто не
    /// блокував, і блокувати не міг: межа ріже за АДРЕСОЮ, ще до того, як
    /// стане відомо, чи існує назване ім'я взагалі. Наслідок не в тексті:
    /// користувач дзвонить у підтримку через блокування, якого немає, а
    /// адміністратор «розблоковує» обліковку, яка не заблокована.
    ///
    /// ⚠ Суб'єкт відмови різний, і саме тому коди різні. <c>0423</c> —
    /// ОБЛІКОВКА, зачинена політикою невдалих спроб (<c>ФВ-6.4a</c>); минає
    /// розблокуванням або строком блокування. <c>0429</c> — АДРЕСА, що
    /// вичерпала хвилинну межу; минає сама, і скільки чекати, сказано в
    /// <c>Retry-After</c>. Дія користувача в цих двох станах різна, тож
    /// один код на обидва позбавляв би його єдиної підказки, що робити.
    ///
    /// ⚠ Цифри коду — наш HTTP-статус (<c>ECR-&lt;ДОМЕН&gt;-&lt;HTTP&gt;</c>),
    /// і 429 тут читається так само, як 423 поруч.
    /// </remarks>
    public const string TooManyLoginAttempts = "ECR-AUTH-0429";
    ...
    /// <summary>
    /// Обчислення прив'язується до колонки, яка НЕ обчислювана: формула
    /// шаблону або вихід методології на колонці ручного вводу.
    /// </summary>
    /// <remarks>
    /// ⛔ Дзеркало до <c>ECR-TMPL-4226</c> («обчислювана колонка без джерела»):
    /// там тип колонки обіцяє обчислення, а обчислювати нічим; тут навпаки —
    /// обчислення є, а тип колонки його не оголошує. Наслідок другого гірший
    /// за перший: <c>ColumnDef.IsComputed</c> хибний, тож
    /// <c>EditRules.CanEdit</c> ПУСКАЄ оператора в комірку. Він уводить число,
    /// бачить його на екрані — і найближчий перерахунок мовчки кладе туди
    /// своє. Ні помилки, ні попередження; єдиний слід — рядок аудиту з
    /// автором «система».
    ///
    /// ⚠ Номер 4227, а не 4226: 4226 зайнятий дзеркальною перевіркою
    /// публікації — два коди на два різні стани, бо й виправлення в них різні
    /// (тут — тип колонки чи саме обчислення, там — відсутнє джерело).
    /// </remarks>
    ...
    /// <summary>
    /// Невалідний запит до методології (<c>ECR-CALC-0422</c>): заголовок
    /// нейтральний, випадок — у <c>messageKey</c>.
    /// </summary>
