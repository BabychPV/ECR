...
public enum OutOfWindowBehavior : byte
{
    ...
    /// <remarks>
    /// ⚠ На сервері <b>не відрізняється</b> від <see cref="ReadOnly"/>:
    /// обидва — «заборонити» з <c>ФВ-2.16</c>, а різниця між «сховати» і
    /// «показати сірим» суто презентаційна, і вимога її не потребує.
    /// Рішення про доступ (<c>EditDecision</c>) поведінки не несе, тож
    /// клієнт малює обидва однаково. Записано тут навмисно: інакше
    /// наступний читач вирішить, що приховування працює, і покладеться на
    /// нього як на приховування ДАНИХ, яким воно не є.
    /// </remarks>
    Hide = 0,
