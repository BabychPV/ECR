...
public sealed class UnitConverter
{
    ...
    public decimal Convert(decimal value, UnitSpec from, UnitSpec to, UnitConversion? explicitConversion)
    {
        ...
        // тихо неправильним числом: при `from.FactorToBase == 0` вираз
        // `(value × 0) + offset` згортається до КОНСТАНТИ, і кожне вхідне
        // значення конвертується в те саме число без жодної помилки.
