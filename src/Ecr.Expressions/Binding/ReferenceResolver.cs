...
public sealed class ReferenceResolver(TemplateVersionSnapshot snapshot)
{
        ...
        CellReferenceNode node, TableDef table, int? currentColumnDefId, List<ExpressionDiagnostic>? diagnostics)
    {
        ...
        // Lookup-колонка в арифметиці — типова помилка переносу з Excel: там
        // код довідника «числоподібний». Для числа треба поле реєстру (02b §5).
        if (column.DataType == CellDataType.Lookup)
        {
            Report(diagnostics, node,
                $"Колонка '{column.Code}' зберігає посилання на реєстр; для числа використовуйте поле реєстру.");
        }
