...
public sealed class SnapshotWorkbookWriter : ISnapshotWorkbookWriter
{
    ...
    public async Task<Stream> WriteAsync(SnapshotWorkbook workbook, CancellationToken ct)
    {
        ...
        {
            // ⛔ R9: заголовок — ПІДПИС колонки, уже вибраний мовою запиту там
            // само, де його бере екран (`SnapshotColumn.Name`). Другий вибір
            // мови тут розійшовся б із першим мовчки; для опису без назв підпис
            // дорівнює коду, тобто книга лишається тією самою, що й до R9.
            sheet.Cell(1, c + 1).Value = workbook.Columns[c].Name;
        }
    ...
    /// <remarks>
    /// ⚠ Без каталогу, на відміну від заголовків колонок (<c>R9</c>): ті
    /// приходять уже підписаними в <see cref="SnapshotColumn.Name"/>, а цей
    /// рядок малює сам писар, і мови користувача він не бачить. Підпис потрібен,
    /// щоб рядок чисел під таблицею не прочитався як ще один рядок звіту.
    /// </remarks>
