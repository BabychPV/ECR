    ...
    Domain.Abstractions.IClock clock) : IRecalculationJob
{
    ...
    private async Task<int> FormulasAsync(RecalculationRequest request, CancellationToken ct)
    {
        ...
        //
        // ⚠ Q-329: коли `SheetDefId` заданий, скоуп звужується до періодів, у
    ...
    {
        // ⚠ Q-329: методологія прив'язана до `TableDefId`, тобто до конкретної
...
/// Аркуш; <c>null</c> — увесь документ (поведінка до Q-329). Звужує лише те, ЩО
