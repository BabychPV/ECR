...
public static class PublishChecks
{
        ...
        TableDef table, IReadOnlySet<int>? boundColumnIds, List<ExpressionDiagnostic> diagnostics)
    {
        ...
        // ⛔ ЛИШЕ `Formula`, а НЕ будь-яка обчислювана (`IsComputed`). Спершу
        // тут стояло `IsComputed`, і це виявилося занадто суворо: воно
        // забороняло публікувати структуру, доки кожна `Calculated`-колонка вже
        // має прив'язку. Але джерело `Calculated`-колонки живе ПОЗА версією
        // шаблону (`cfg.CalculationBinding`), і заводить його ІНША людина —
        // методолог, — уже після того, як структуру опубліковано: методологію
        // немає до чого прив'язувати, доки немає опублікованого шаблону.
        //
        // ⚠ Знайдено не міркуванням, а падінням:
        // `CalculationOrchestratorConcurrencyScenarios` заводить дві
        // `Calculated`-колонки, публікує версію й лише потім створює дві
        // методології та прив'язує їх. Це не хиба сценарію — це штатний
        // порядок ролей, і перевірка його забороняла.
        //
        // ⚠ Для `Formula` вимога лишається жорсткою, і саме тому вона чесна:
        // формула колонки живе В ЦІЙ САМІЙ ВЕРСІЇ, тож у момент публікації
        // відомо остаточно — вона є або її немає, і «з'явиться пізніше» тут не
        // буває.
        //
        // ⚠ Ціна звуження названа: `Calculated`-колонка без прив'язки
        // публікується й далі, і в опублікованій формі це порожня клітинка,
            ...
            !c.IsDeleted && c.DataType == Domain.Enums.CellDataType.Formula))
        {
            ...
            diagnostics.Add(new ExpressionDiagnostic(
                ...
                + "рахуватися формулою шаблону — а формули в цій версії немає. В опублікованій "
