...
public interface IMethodologyDraftStore
{
    ...
    /// <summary>
    /// Методології <b>разом із версіями</b> — одним запитом на весь перелік.
    /// </summary>
    /// <param name="methodologyIds">
    /// Кого читаємо; порожній перелік — УСІ активні. Ті самі два значення
    /// порожнього набору, що й у переліку адміністрування.
    /// </param>
    /// <param name="ct">Токен скасування.</param>
    /// <returns>Знайдені методології за кодом; відсутніх у результаті просто немає.</returns>
    /// <remarks>
    /// ⛔ Рядок <c>RD-06</c>. Доти перелік робив
    /// <see cref="ListActiveIdsAsync"/> і далі <see cref="FindAsync"/> НА КОЖЕН
    /// ідентифікатор: 1 + N звернень до бази на відкриття екрана, де N — усі
    /// активні методології корпусу. Тут запит один, і число не залежить від N.
    ///
    /// ⛔ Результат <b>невідстежуваний</b> — і цим метод принципово
    /// відрізняється від <see cref="FindAsync"/>, чия відстежуваність
    /// документована як несуча (доданій чернетці потрібно чим зберегтися).
    /// Перелік нічого не змінює, тож кожна прочитана версія в
    /// <c>ChangeTracker</c> — це лише пам'ять і ризик випадкового запису.
    /// Саме тому це ОКРЕМИЙ метод, а не «FindAsync у циклі одним запитом».
    ///
    /// ⚠ Порядок — за <c>Code</c>, той самий, що давав
    /// <see cref="ListActiveIdsAsync"/>. Коли ідентифікатори задані явно,
    /// порядок запиту відновлює викликач: сховище не знає, чому він такий.
    /// </remarks>
    public Task<IReadOnlyList<Methodology>> ListWithVersionsAsync(
