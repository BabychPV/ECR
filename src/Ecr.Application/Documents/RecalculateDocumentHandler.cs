    ...
    IDocumentStore documents)
{
    ...
    /// Аркуш; <c>null</c> — увесь документ (поведінка до Q-330). Реальний
    /// перерахунок ОДНОГО аркуша (директива паритету зі старою системою,
    /// прогалина 2, Q-327 → Q-330): звужує ЦІЛІ запису до таблиць цього
        ...
        long documentId, PeriodKey periodKey, int? sheetDefId, CancellationToken ct)
    {
        ...
        // ⛔ Q-330: аркуш мусить входити в СКЛАД документа — той самий гейт,
    ...
    /// <para>
    /// ⚠ Q-330: НЕ пара «документ + аркуш» — умисно. Кожен прогін перемикає
