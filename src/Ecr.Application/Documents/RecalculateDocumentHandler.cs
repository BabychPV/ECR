    ...
    IDocumentStore documents)
{
    ...
    /// Аркуш; <c>null</c> — увесь документ (поведінка до Q-329). Реальний
    /// перерахунок ОДНОГО аркуша (директива паритету зі старою системою,
    /// прогалина 2, Q-327 → Q-329): звужує ЦІЛІ запису до таблиць цього
        ...
        long documentId, PeriodKey periodKey, int? sheetDefId, CancellationToken ct)
    {
        ...
        // ⛔ Q-329: аркуш мусить входити в СКЛАД документа — той самий гейт,
    ...
    /// <para>
    /// ⚠ Q-329: НЕ пара «документ + аркуш» — умисно. Кожен прогін перемикає
