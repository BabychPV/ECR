...
test('ФВ-14.18: знеколірення справді прибирає колір', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    // ⛔ Фікстура стоїть у ЦІЛИХ координатах, а не там, куди її покладе потік
    // документа. Приклеєна в кінець `<body>`, вона діставала дробове зміщення
    // від усього, що намальовано вище (виміряно на стенді: `y = 1603.0625`), і
    // при `deviceScaleFactor: 2` обрізки двох сусідніх блоків округлювалися
    // по-різному — 200×48 проти 200×50. Це ламало сам вимір: `structuralDifference`
    // порівнює пікселі попарно і на різних розмірах не може сказати нічого,
    // тому й падає з «Обрізки різного розміру».
    //
    // ⚠ `position: fixed` з цілими `top`/`left` прибиває обидва блоки до сітки
    // пристроєвих пікселів незалежно від висоти сторінки під ними. Твердження
    // від цього не слабшає: перевіряється та сама пара «два фони без форми»,
    // і поріг лишається той самий — змінилося лише те, що вимір тепер узагалі
    // визначений.
    host.setAttribute(
      'style',
      'position:fixed;top:0;left:0;z-index:9999;background:#000;line-height:24px;font-size:12px',
    );
    host.innerHTML = `
