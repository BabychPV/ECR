...
function crumbSignature(chain: readonly CrumbEntry[]): string {
  ...
  //
  // ⛔ Заглушка для `null` пишеться escape-послідовністю `\u0000`, а НЕ
  // самим байтом 0x00: сирий NUL у джерелі змушує ripgrep вважати файл
  // двійковим і мовчки пропускати його при пошуку по каталогу. `rg
  // crumbSignature src/Ecr.Web/src/app` тоді не знаходив НІЧОГО — файл, у
  // якому ця функція оголошена. Рядок на виході однаковий побайтно.
  return chain
    .map(
      (entry) =>
