...
describe('ExpressionsPage і пагінована відповідь версій шаблону (Q-275)', () => {
  it(
    ...
    async () => {
      ...
      /*
       * ✎ ЗМІНА ПОВЕДІНКИ, а не підгін локатора. Тут стояло
       * `'1.0.0.0 · Published'` — тобто перевірялося, що в підпис варіанта
       * доходить КОД СЕРВЕРА. Відтепер статус версії всюди береться з
       * каталогу (`statusKey('version', …)`), як і в `StatusBadge`: та сама
       * версія не може називатися в таблиці однією мовою, а у випадному
       * списку — англійським `Published`.
       *
       * ⚠ Предмет тесту не змінився й не ослаб: він про те, що сторінка НЕ
       * падає на пагінованій відповіді (`Q-275`) і показує версію. Підпис
       * лишається в тому ж вузлі, лише його друга половина тепер із
       * каталогу — без завантаженого каталогу `t()` чесно повертає
       * позначений ключ, і саме з ним тут і звіряємося.
       */
      expect(
        await screen.findByText('1.0.0.0 · ⟦status.version.Published⟧', {}, { timeout: 400_000 }),
      ).toBeDefined();
    },
    400_000,
  );
