          ...
          */}
          <Suspense fallback={null}>
            <DateInput
              ...
              /*
               * ⚠ Причина названа МОМЕНТОМ, а не окремим реченням, і це
               * вимушено: власного рядка «дата вже минула» в каталозі немає, а
               * завести його звідси нічим — сторож
               * `EndpointCoverageTests.Кожен_рядок_якого_просить_клієнт_є_в_каталозі`
               * перевіряє кожен `t('…')` по `09-seed.sql`, і ключ без рядка
               * зробив би гейт `test` червоним. Тому тут той самий рядок, яким
               * сторінка вже підписує строк у таблиці (`periods.reopenedUntil`,
               * «open until {until}»): у червоному полі він показує момент, на
               * який людина щойно попросила відкрити період, — і цього досить,
               * щоб побачити, що він у минулому.
               *
               * ⛔ Названа прогалина: окремий рядок `periods.reopenUntilPast`
               * каталогу потрібен, і поки його немає, причина слабша, ніж
               * могла б бути. Це borrow, а не рішення.
               */
              error={
                reopenUntilPast
                  ? t('periods.reopenedUntil', { until: formatDateTime(reopenUntilIso) })
                  : undefined
              }
            />
          </Suspense>
