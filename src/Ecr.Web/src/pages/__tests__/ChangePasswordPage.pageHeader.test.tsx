...
describe('ChangePasswordPage: PageHeader (Q-261)', () => {
  // ⚠ Раніше тут стояло «цей тест — ПЕРШИЙ навмисно»: `announceRoute`
  // (`RouteAnnouncer.tsx`) дедуплікує за текстом у модульній змінній `last`,
  // усі чотири тести монтують ту саму сторінку з тим самим заголовком
  // (`password.title`), і на будь-якому іншому порядку цей тест бачив
  // порожню область оголошень. Порядок тестів — не механізм: у прогоні з
  // `--sequence.shuffle` файл падав приблизно на кожному другому сіді.
  // Причину прибрано в продукті — знята область оголошень скидає `last`
  // (`RouteAnnouncer.tsx`), тож кожен рендер тут починає з чистого стану
