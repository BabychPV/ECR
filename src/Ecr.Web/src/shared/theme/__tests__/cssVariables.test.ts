...
describe('UI-01: резолвер віддає токени макета обом схемам', () => {
  ...
  it.each(Schemes)('схема «%s»: межу поля вводу видно на кожній поверхні', (scheme) => {
    const vars = resolve(scheme);

    /*
     * ⛔ Ця перевірка з'явилася після того, як `theme.ts` носив виміряний
     * дефект у ВЛАСНОМУ коментарі («обидва нижчі за 3:1») і жоден із 1200+
     * тестів його не бачив: набір міряв `faint`, `focus`, лінії станів і
     * текст — усе, крім межі КЕРУВАННЯ. Названий у коментарі дефект, якого
     * ніхто не міряє, живе рівно доти, доки коментар не перестануть читати.
     *
     * ⚠ Міряється `--mantine-color-default-border`, а не `--ecr-border-strong`:
     * на екрані межу поля малює саме перша, і якби резолвер колись перестав
     * виводити її з `borderStrong`, перевірка на `--ecr-*` лишилася б зеленою
     * при невидимій межі. Те, що вони сьогодні рівні, стереже окрема `it`
     * нижче («власні змінні Mantine перебиті нашими»).
     *
     * ⚠ `AA.nonText` (3:1) — поріг графічного елемента керування (WCAG 1.4.11),
     * а не послаблення заради проходження: межа поля не несе тексту.
     */
    for (const bg of Backdrops) {
      const value = contrast(readVar(vars, '--mantine-color-default-border'), readVar(vars, bg));

      expect(value, `${scheme}: межа поля вводу на ${bg}`).toBeGreaterThanOrEqual(AA.nonText);
    }
  });
