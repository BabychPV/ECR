 ...
 *
 * ⚠ Модуль спершу імпортується напряму, і лише потім рендериться. Інакше
 * холодний `import()` ішов усередині 1 с `findByRole`: файл стартує в першу
 * секунду повного прогону, коли всі воркери разом тягнуть модулі, і
 * 2026-09-21 під навантаженням прогрів упав саме тут — «Unable to find
 * role="button" and name "Прогрів копіювання"», а з ним і весь файл.
 * Рендер нижче лишається: він прогріває сам `React.lazy`, а не модуль.
 */
beforeAll(async () => {
  await import('@/shared/ui/CorrelationCopy');
