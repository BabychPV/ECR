 ...
 *
 * ✎ 2026-09-20. Додано `messageKey`, і це не косметика фікстури. Відколи діє
 * рішення людини «українську прибрати — має бути залежно від обраної мови»,
 * подробиця показується ЛИШЕ з цією ознакою: сервер ставить її тоді, коли
 * `detail` зібрано з каталогу мовою користувача. Без неї цей тест перевіряв
 * би екран, на якому подробиці навмисно немає — і не помітив би різниці.
 *
 * ⚠ «Немає доступу до документа» — серед головних шляхів, які сервер уже
 * позначає, тож фікстура описує реальність, а не бажане.
 */
function forbidden(): EcrApiError {
  return new EcrApiError({
    ...
    extensions2: { messageKey: 'err.ECR-AUTH-0403.forbidden' },
  });
}
