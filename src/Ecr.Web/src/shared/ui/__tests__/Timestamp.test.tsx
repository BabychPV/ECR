...
describe('Timestamp: видиме читабельне, точне — в розмітці', () => {
  ...
  it('precise додає секунди, і саме цим відрізняється від дефолту', () => {
    /*
     * ⛔ Дефолт навмисно БЕЗ секунд: зайва точність на екрані читається як
     * важливість і змушує вчитуватися в число, яке не має змісту. `precise`
     * заведено під один конкретний випадок — мітки телеметрії в перегляді
     * мапінгу, де сусідні виміри відрізняються саме секундою.
     *
     * ⚠ Обидва твердження обов'язкові: перше — що секунди Є, друге — що без
     * прапорця їх НЕМАЄ. Без другого перевірка лишилася б зеленою, якби
     * `precise` перестав на щось впливати.
     */
    show(<Timestamp value={Moment} precise />);

    const precise = document.querySelector('time')?.textContent ?? '';

    expect(precise).toMatch(/\d{1,2}:\d{2}:\d{2}/);
    expect(precise).not.toBe(formatDateTime(Moment));
  });
