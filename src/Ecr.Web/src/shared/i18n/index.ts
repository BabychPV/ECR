...
/**
 * Переклад із ВЛАСНИМ запасним варіантом замість позначеного ключа.
 *
 * ⛔ Навіщо окремо від `t()`: позначка `⟦ключ⟧` правильна там, де ключ
 ...
 * геть незрозуміла позначка. Це погіршення, а не сигнал.
 *
 * ⚠ Промах СЮДИ не потрапляє в `report()` навмисно: для похідного від даних
 * ключа «його немає» — очікуваний стан, і скарга в консоль була б тим самим
 * шумом, який `pendingReports` вище й прибирає.
 */
export function tOr(
  key: string,
  fallback: string,
  ...
  if (!hasText(key)) return fallback;
