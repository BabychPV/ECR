...
export function buildMonarchLanguage(options: LanguageOptions): MonarchLanguage {
  ...
  // змінює результат.
  //
  // ⚠ Межа слова перед переглядом уперед НЕ декоративна. Без неї зразок
  // відкочується: на `!SUM(x)` він скорочує ім'я до `SU`, і тоді «наступний
  // символ не дужка» стає правдою — заперечення фарбувалося б як посилання на
  // формулу з іменем `SU`. Редактор показував би інший зміст, ніж бачить
  // парсер, і саме там, де різниця змінює результат.
