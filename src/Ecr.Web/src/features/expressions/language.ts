...
export function buildMonarchLanguage(options: LanguageOptions): MonarchLanguage {
  // ⚠ Верхній регістр: `ignoreCase: true` зводить обидва боки `cases` до
  // одного написання, тому перелік має бути в одному регістрі, щоб зіставлення
  // взагалі спрацювало.
  //
  // ⚠ У діалекті методологій регістр імені ЗНАЧУЩИЙ (`Pow` є, `POW` немає), і
  // тут ця різниця не відтворюється: `POW(2,3)` пофарбується як відома
  // функція. Це свідома межа файлу, а не недогляд — тут вирішується лише
  // ЛЕКСИЧНИЙ КЛАС токена, а про правильність каже сервер і каже словами
  // («регістр значущий, і пишеться воно 'Pow'»). Розділити регістр у Monarch
  // означало б завести другий перелік ключових слів (`and`/`AND`, `true`/`TRUE`),
  // тобто ще одну правду про мову — рівно те, проти чого цей файл написаний.
  ...
  return {
    ...
    tokenizer: {
      root: [
        ...
        [/CST\./, 'constant'],
        // ⛔ Крапки — ЧАСТИНА імені (`I.6`, `02b` §3.4). Без хвоста
        // `@GCV.HSE400_FG_Makat_Methane` фарбувався б як `@GCV` плюс щось
        // інше, і редактор показував би інший зміст, ніж бачить парсер: там
        // це ОДНЕ посилання на аргумент. У корпусі таких імен 2534.
        //
        // ⚠ Крапка продовжує ім'я лише перед літерою або підкресленням —
        // дослівно як `Lexer.ReadDottedName`. Інакше `@X.5` пофарбувалося б
        // до кінця числа, а лексер зупиниться на крапці.
