 ...
 *
 * ⛔ Ширше за `isNumericColumn` (`cellValue.ts`) рівно на `Int`, і вужче за
 * «усе, що є числом» рівно на `Lookup`. Перше: `isNumericColumn` відповідає на
 * питання ПОКАЗУ («малювати як число»), а лічильник у цілій колонці
 * додається так само осмислено, як і десяткове. Друге: комірка `Lookup`
 * тримає `ValueRegistryEntryId` (`edits.coerce`) — число, сума якого
 * не означає нічого, а виглядає як вимірювання.
 */
