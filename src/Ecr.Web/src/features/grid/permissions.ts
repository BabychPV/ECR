...
/**
 * Рядок із ключа комірки — обернене до `cellKey`.
 *
 * ⛔ Живе РЯДОМ із `cellKey`, а не на місці використання: два визначення того,
 * як склеєний ключ, розійшлися б на першій же зміні роздільника, і розійшлися
 * б мовчки — обернене перетворення на неправильному ключі не падає, воно
 * просто повертає не той рядок.
 *
 * ⚠ Ділиться за ПЕРШИМ роздільником: `rowKey` — GUID у форматі `N` (сервер
 * видає його сам, `ФВ-3.2`), двокрапки в ньому не буває, а от код колонки
 * теоретично може містити що завгодно, і хвіст має лишитися хвостом.
 */
export function rowKeyOfCellKey(key: string): string {
