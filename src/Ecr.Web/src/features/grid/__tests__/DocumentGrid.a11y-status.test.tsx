...
vi.mock('../useCellPatch', async () => {
  ...
  return {
    ...
    useCellPatch: () => ({
      ...
      // ⚠ `BE-05`: стежити нема за чим — предмет ЦЬОГО файлу лише індикатор
      // автозбереження. З непорожнім ідентифікатором поруч зʼявився б другий
      // `role="status"` (рядок перерахунку), і `getByRole('status')` нижче
      // упав би на «знайдено кілька елементів» — на факті, до якого Q-264
      // не має стосунку.
