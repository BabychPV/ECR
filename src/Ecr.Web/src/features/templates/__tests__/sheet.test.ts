...
import { emptyDraft, whyCannotSave } from '../sheet';

/**
 * Той самий дефект, що й `table.test.ts` (Q-336, `table.ts`): «Add sheet»
 * казало «дай коду код» навіть ПІСЛЯ введення коду, якщо той містив
 ...
 * повернуть те саме.
 */
describe('whyCannotSave (SheetDraft)', () => {
  ...
  it('чинний код і назва — нічого не блокує', () => {
    expect(
      whyCannotSave({ ...emptyDraft(1), code: 'VALID_CODE', nameL10n: { en: 'Sheet' } }),
    ).toBeNull();
  });
});
