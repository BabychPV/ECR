...
import { MethodologyConstantUsage } from '@/features/methodologies/ConstantUsage';
import { testTheme } from '@/test/render';

/**
 * `MethodologyConstantUsage` — «де використано» константи версії (ФВ-8.14).
 *
...
const Refusal = {
  ...
  status: 500,
  detail: 'залежності константи прочитати не вдалося',
  errorCode: 'ECR-SYS-0500',
  correlationId: 'cid-constant-usage-1',
  ...
  render(
    <MantineProvider theme={testTheme}>
      <QueryClientProvider client={client}>
        <MemoryRouter>
          <MethodologyConstantUsage methodologyId={1} versionId={2} code="K1" />
        </MemoryRouter>
      </QueryClientProvider>
    </MantineProvider>,
  );
...
describe('MethodologyConstantUsage', () => {
  it(
    // ⛔ Мутаційний доказ: якби афорданс ховався при `total: 0` (елемент без
    // даних не малюється — як робить L10-подібний паттерн деінде), цей блок
    // був би порожнім. Тут навпаки: список завжди щось малює.
  ...
  it('успіх із записами — формула й прив’язка методики у переліку', async () => {
    mockUsage({
      total: 2,
      items: [
  ...
  it('відмова — причина з кодом, і НЕ «ніде не використано»', async () => {
    ...
    expect(alert.textContent ?? '').toContain('залежності константи прочитати не вдалося');
