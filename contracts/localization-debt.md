...
⛔ Один кидок `ECR-DOC-0404` лишився БЕЗ ключа свідомо:
`Repository<T,TId>.GetAsync` (`src/Ecr.Infrastructure/Persistence/Repository.cs`).
Він узагальнений на п'ять типів сутностей і п'ять кодів, а його текст називає
ім'я КЛАСУ .NET (`TemplateVersion з ідентифікатором 5 не знайдено`) — підставити
таке в локалізований шаблон означало б показати користувачеві внутрішнє ім'я
типу під виглядом перекладу. Правильне лікування — окремі повідомлення на
сутність, а це переробка самого методу, не додавання ключа.
...
| `src/Ecr.Infrastructure/Persistence/Repository.cs` | 1 |
