...
public sealed class PublishChecksTests
{
    ...
    public void Колонка_Calculated_без_прив_язки_публікацію_НЕ_блокує()
    {
        ...
        //
        // ⛔ Ця перевірка тепер існує —
        // `ConsistencyCheckJob.UnboundCalculatedColumnsAsync`, знахідка
        // `UNBOUND_CALCULATED_COLUMN` (доводять
        // `ConsistencyCheckJobDetectionTests.Виявляє_колонку_Calculated_без_жодної_прив_язки`
        // і парний до нього). ЦЕЙ тест лишається сторожем протилежного боку:
        // він доводить, що вона не заповзла назад у публікацію структури, бо
        // саме там вона вже одного разу зламала штатний порядок ролей (`#276`
        // → `#284`).
